...
(define (account-credit! amount account)
  (set-account-balance! account (+ amount (account-balance account))))
...
(define (vector-copy-resize size vec)
  (define i 0)
  (define (loop-helper i)
    (if (< i (vector-length vec))
        (vector-ref vec i)
        #false
    )
  )
  (build-vector size loop-helper)
)
;; ^ FILL IN YOUR CODE HERE
...
(define (find-largest-account accounts)
  (define element 0)
  (define mxtmp (vector-ref accounts element))
  (for ([element (build-vector (vector-length accounts) (lambda (i) i))])
    (if (< (account-balance mxtmp) (account-balance (vector-ref accounts element)))
        (set! mxtmp (vector-ref accounts element))
        (void)
    )
  )
  mxtmp
